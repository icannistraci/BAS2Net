...
def compute_ave_MAE_of_methods(gt_name_list, rs_dir_lists):
    ...
    for i in range(0, num_gt):
        ...
        gt_name = os.path.basename(gt_name_list[i])  # get the file name of the ground truth
        tmp_mae = 0.0
        try:
            rs1 = io.imread(rs_dir_lists[0] + gt_name)  # read the corresponding mask of each method
        except IOError:
            print('ERROR: Couldn\'t find the following file:', rs_dir_lists[0]+gt_name)
            continue
        try:
            rs2 = io.imread(rs_dir_lists[1] + gt_name)  # read the corresponding mask of each method
        except IOError:
            print('ERROR: Couldn\'t find the following file:', rs_dir_lists[1]+gt_name)
            continue
        try:
            tmp_mae1 = compute_mae(gt, rs1)  # compute the mae
            tmp_mae2 = compute_mae(gt, rs2)  # compute the mae
        except IOError:
            print('ERROR: Fails in compute_mae!', gt_name_list[i], rs_dir_lists[0] + gt_name, rs_dir_lists[1] + gt_name)
            continue
        mae[i][0] = tmp_mae1
        gt2rs[i][0] = 1.0
        mae[i][1] = tmp_mae2
...
def compute_PRE_REC_FM_of_methods(gt_name_list, rs_dir_lists, beta=0.3):
    ...
    for i in range(0, num_gt):
        ...
        try:
            rs1 = io.imread(rs_dir_lists[0] + gt_name)  # read the corresponding mask from each method
            rs1 = mask_normalize(rs1) * 255.0  # convert rs to [0,255]
        except IOError:
            print('ERROR: Couldn\'t find the following file:', rs_dir_lists[0]+gt_name)
            continue
        try:
            rs2 = io.imread(rs_dir_lists[1] + gt_name)  # read the corresponding mask from each method
            rs2 = mask_normalize(rs2) * 255.0  # convert rs to [0,255]
        except IOError:
            print('ERROR: Couldn\'t find the following file:', rs_dir_lists[1]+gt_name)
            continue
        try:
            pre1, rec1 = compute_pre_rec(gt, rs1, mybins=np.arange(0, 256))
            pre2, rec2 = compute_pre_rec(gt, rs2, mybins=np.arange(0, 256))
        except IOError:
            print('ERROR: Fails in compute_pre_rec!', gt_name_list[i], rs_dir_lists[0] + gt_name, rs_dir_lists[1] + gt_name)
            continue
